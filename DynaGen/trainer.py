#Code based on https://github.com/lucidrains/imagen-pytorch
...
from DynaGen.DynaGen import DynaGen, NullUnet
from DynaGen.elucidated_DynaGen import ElucidatedDynaGen
from DynaGen.data import cycle
...
class DynaGenTrainer(nn.Module):
    locked = False
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        assert not DynaGenTrainer.locked, 'DynaGenTrainer can only be initialized once per process - for the sake of distributed training, you will now have to create a separate script to train each unet (or a script that accepts unet number as an argument)'
        assert exists(imagen) ^ exists(imagen_checkpoint_path), 'either DynaGen instance is passed into the trainer, or a checkpoint path that contains the DynaGen config'
        ...
        assert isinstance(imagen, (DynaGen, ElucidatedDynaGen))
        ema_kwargs, kwargs = groupby_prefix_and_trim('ema_', kwargs)

        # elucidated or not

        self.is_elucidated = isinstance(imagen, ElucidatedDynaGen)
        ...
        DynaGenTrainer.locked = self.is_distributed
        ...
        self.is_distributedalid_dl_iter = None
        self.valid_dl = None
    ...
    def load(self, path, only_model = False, strict = True, noop_if_not_exist = False):
        ...
        if version.parse(__version__) != version.parse(loaded_obj['version']):
            self.print(f'loading saved DynaGen at version {loaded_obj["version"]}, but current package version is {__version__}')
        ...
        self.print(f'DynaGen checkpoint loaded from {path}')
        return loaded_obj

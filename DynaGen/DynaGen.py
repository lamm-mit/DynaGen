...
from DynaGen.DynaGen_video import Unet3D, resize_video_to
...
# main DynaGen ddpm class, which is a cascading DDPM from Ho et al.

class DynaGen(nn.Module):
    def __init__(
        self,
        unets,
        *,
        ...
        self.encode_text = None   #if text based encoding is used, need to code it up here, can use text_encoder_name to define which one used
        # construct unets
    ...
    def p_mean_variance(
        ...
        *,
        ...
        assert not (cond_scale != 1. and not self.can_classifier_guidance), 'dynagen was not trained with conditional dropout, and thus one cannot use classifier free guidance (cond_scale anything other than 1)'
    ...
    def sample(
        ...
        assert not (self.condition_on_text and not exists(text_embeds)), 'text or text encodings must be passed into dynagen if specified'
        assert not (not self.condition_on_text and exists(text_embeds)), 'dynagen specified not to be conditioned on text, yet it is presented'

...
class Unet3D(nn.Module): #for dynamical field prediction
    def __init__(
        self,
        *,
        dim,
        ...
        out_dim = None,
        dim_mults=(1, 2, 4, 8),
        cond_images_channels = 3, #depth of microstructure encoding, i.e. input microstructure from which predictions are made
        channels = 3,
        channels_out = None,
        attn_dim_head = 64,
        attn_heads = 8,
    ...
    def forward(
        ...
        *,
        ...
        text_embeds = None, #not used here 
        text_mask = None, #not used here
        cond_images = None, #microstructures are encoded via cond_images
        self_cond = None,
        cond_drop_prob = 0.
    ):
        ...
        if exists(cond_images):
            assert cond_images.shape[1] == self.cond_images_channels, 'the number of channels on the conditioning data you are passing in does not match what you specified on initialiation of the unet'
            cond_images = resize_video_to(cond_images, x.shape[-1])
            x = torch.cat((cond_images, x), dim = 1) #concatenate microstructure encoding in chennel dimension 
